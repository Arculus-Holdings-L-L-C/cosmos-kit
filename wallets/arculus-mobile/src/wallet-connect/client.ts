...
export class ArculusClient extends WCClient {
  arculus: Keplr | undefined = undefined;
  ...
  initKeplrWCClient(
    ...
    this.arculus = new KeplrWalletConnectV2(
      signClient,
      options
    ) as unknown as Keplr;

    // This is for Arculus App Deep Linking
    // https://github.com/chainapsis/arculus-wallet/blob/120a7a32b54a0976ea3d386b3105e2c41eaf41bf/packages/wc-client/src/index.ts#L209
    localStorage.setItem(
      ...
      JSON.stringify(
        this.env?.os === 'android'
          ? {
              name: 'Arculus',
              href: 'intent://wcV2#Intent;package=com.chainapsis.arculus;scheme=arculuswallet;end;',
            }
          : {
              name: 'Arculus',
              href: `arculuswallet://wcV2`,
            }
      )
    );
  }
  ...
  async enable(chainIds: string | string[]): Promise<void> {
    if (!this.arculus) {
      throw new Error('Arculus Wallet is not initialized');
    }

    await this.arculus.enable(chainIds);
  }
  ...
  async getAccount(chainId: string): Promise<WalletAccount> {
    ...
    const key = await this.arculus.getKey(chainId);
    return {
      username: key.name,
  ...
  getOfflineSignerAmino(chainId: string) {
    ...
    return this.arculus.getOfflineSignerOnlyAmino(chainId);
  }
  ...
  getOfflineSignerDirect(chainId: string) {
    ...
    return this.arculus.getOfflineSigner(
      chainId
  ...
  async signAmino(
    ...
    return await this.arculus.signAmino(chainId, signer, signDoc, signOptions);
  }
  ...
  async signDirect(
    ...
    return await this.arculus.signDirect(
      chainId,
      signer,
      {
        ...signDoc,
  ...
  async signArbitrary(
    ...
    return await this.arculus.signArbitrary(chainId, signer, data);
  }
}
